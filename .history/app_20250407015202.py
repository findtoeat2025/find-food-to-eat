...
def log_user_data(user_agent, lat, lng, address):
    ...
    is_duplicate = is_duplicate_visit(user_agent, lat, lng)
    
    c.execute('''INSERT INTO user_logs 
                 (timestamp, user_agent, latitude, longitude, address, is_duplicate) 
                 VALUES (?, ?, ?, ?, ?, ?)''',
              (datetime.now().isoformat(), user_agent, lat, lng, address, 1 if is_duplicate else 0))
...
def get_user_logs():
    try:
        ...
        # Get all logs, excluding duplicates
        c.execute('''SELECT * FROM user_logs 
                    WHERE is_duplicate = 0 
                    ORDER BY timestamp DESC''')
        logs = c.fetchall()
        
        # Get statistics
        c.execute('SELECT COUNT(DISTINCT user_agent) FROM user_logs WHERE is_duplicate = 0')
        unique_browsers = c.fetchone()[0]
        
        # Get total visits (including duplicates)
        c.execute('SELECT COUNT(*) FROM user_logs')
